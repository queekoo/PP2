...
BLACK = (0, 0, 0)
WHITE = (255, 255, 255)
GOLD  = (255, 215, 0)   # Color for the coin
...
COINS = 0       # Extra task: counts how many coins the player collected
...
coin_image = pygame.image.load("resources/coin.png").convert_alpha()  # Coin image (Extra Task #1)
...
class Enemy(pygame.sprite.Sprite):
    def __init__(self):
        ...
        # Create a rect (rectangle) matching the image size — used for collision detection
...
class Player(pygame.sprite.Sprite):
    ...
    def move(self):
        ...
        # Move left — but only if the player is not already at the left edge
        if self.rect.left > 0:
            if pressed_keys[K_LEFT]:
                self.rect.move_ip(-5, 0)

        # Move right — but only if the player is not already at the right edge
...
# ============================================================
#  Coin Class — Extra Task #1: randomly appearing coins
# ============================================================
class Coin(pygame.sprite.Sprite):
    def __init__(self):
        super().__init__()
        # Use the dollar.png image from resources (already loaded and scaled above)
        self.image = coin_image
        # Create a rect for the coin — used for collision detection
        self.rect = self.image.get_rect()
        # Spawn the coin at a random horizontal position, just above the screen
        self.rect.center = (random.randint(40, SCREEN_WIDTH - 40), 0)

    def move(self):
        # Coins fall at the same speed as the enemy
        self.rect.move_ip(0, SPEED)

    def is_off_screen(self):
        # Returns True if the coin has gone past the bottom of the screen
...
# Custom event: fires every 1500ms to spawn a new coin on the road (Extra Task #1)
...
while True:
    ...
    for event in pygame.event.get():
        ...
        # Spawn a new coin every 1.5 seconds — Extra Task #1
    ...
    # --- Extra Task #2: Render and display COINS in the top-right corner ---
    coins_text = font_small.render("Coins: " + str(COINS), True, GOLD)
    # Position it so it stays inside the right edge of the screen
    ...
    # --- Check if player collected any coins (collision with coin) ---
    # spritecollide returns list of coins that collided with P1 and removes them (True = dokill)
    collected = pygame.sprite.spritecollide(P1, coins_group, True)
    COINS += len(collected)  # Add the number of collected coins to the counter
